...
redis_handle = redis.Redis(host=environ.get('REDIS_HOST', 'redis'), port=6379,
...
if __name__ == "__main__":
    ...
    global serve_port
    if environ.get('SERVE_PORT'):
      serve_port = int(environ.get('SERVE_PORT'))
      app.run(host='0.0.0.0', port=serve_port, debug=True)
    else:
      raise EnvironmentError('SERVE_PORT environment variable not set')
